    ...
    try {
      ...
      if (result.error) {
        ...
        switch (fbqError.status) {
          ...
          case 'PARSING_ERROR':
          case 'TIMEOUT_ERROR':
            const errorMsgKnown = t('failedToCreateAccount') + ' ' + `${fbqError.status}: ${fbqError.error}.`;
            toast(errorMsgKnown, { type: 'error' });
            break;
          case 'CUSTOM_ERROR':
            const errorMsgUnknown = t('failedToCreateAccountCustomError') + fbqError.error + '.';
            toast(errorMsgUnknown, { type: 'error' });
            break;
          default:
            const errorMsgDefault =
              t('failedToCreateAccountStatus') + ' ' + `${fbqError.status}: ${JSON.stringify(fbqError.data)}.`;
      ...
      toast(t('failedToCreateAccountNoPeriod') + `: ${(error as Error).message}.`, {
        type: 'error',
      });
    }
  };
      ...
      <Controller
        ...
        render={({ field }) => (
          <TextInput
            ...
            required
            {...(errors.name && { color: 'failure' })}
            helperText={(errors.name && t('nameRequired')) || t('enterAccountName')}
          />
        )}
      />
